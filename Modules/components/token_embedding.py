...
class Conv1dEmbedding(nn.Module):
    def __init__(self, in_features: int, d_model: int, kernel_size: int, stride: int, padding: int):
        super().__init__()
        self.cnn = nn.Conv1d(in_channels=in_features,
...
class PatchEmbedding(nn.Module):
    def __init__(self, in_features: int, d_model: int, patch_size: int, patch_stride: int):
        super().__init__()
