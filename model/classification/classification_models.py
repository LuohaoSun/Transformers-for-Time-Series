...
class CustomClassificationModel(ClassificationFramework):
    ...
    def __init__(
        ...
        """
        super().__init__(backbone, head, num_classes, lr, max_epochs, max_steps)
    ...
    def configure_backbone(self) -> nn.Module:
        ...
        class Backbone(nn.Module):
            def __init__(self):
                ...
                self.activation = get_activation_fn(model.hparams["activation"])
...
class PatchTSTClassificationModel(ClassificationFramework):
    ...
    def configure_head(self) -> nn.Module:
        head = MeanLinearHead(
            in_features=self.hparams["d_model"], num_classes=self.hparams["num_classes"]
        )
        return head
